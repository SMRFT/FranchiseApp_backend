...
def check_wallet_balance(franchise_id, new_bill_amount=Decimal("0.00")):
    try:
    ...
    new_share = clean_decimal(new_bill_amount) / Decimal('2.00')

    if (wallet_balance - franchise_share - new_share) < 1000:
        return False, "Insufficient wallet balance (below 1000). Please Recharge."
    ...
    try:
        with transaction.atomic():
            ...
            if existing_patient:
                ...
                patient_data = {
                    "patientname": data.get("patientname", patient.patientname),
                    "age": data.get("age", patient.age),
                    "gender": data.get("gender", patient.gender),
                    "phoneNumber": phone or patient.phoneNumber,
                    "email": data.get("email", patient.email),
                    "city": data.get("city", patient.city),
                    "area": data.get("area", patient.area),
                    "pincode": data.get("pincode", patient.pincode),
                    "dateOfBirth": data.get("dateOfBirth", patient.dateOfBirth),
                    "franchise_id": data.get("franchise_id", patient.franchise_id),
                }
            ...
            existing_billing = Billing.objects.filter(barcode=barcode_id).first()
            ...
            elif existing_billing:
                trf_file_id = existing_billing.trf_file_id
            ...
            # Parse testdetails safely to Python list
            testdetails_val = data.get("testdetails")
            if testdetails_val:
                if isinstance(testdetails_val, str):
                    try:
                        testdetails_val = json.loads(testdetails_val)
                    except Exception:
                        testdetails_val = []
                elif not isinstance(testdetails_val, list):
                    testdetails_val = [testdetails_val]
            else:
                testdetails_val = []

            # Parse payments safely to Python list
            ...
            if existing_billing:
                if existing_billing.billing_status == "Billed":
                    return Response({"error": "Barcode already billed. Cannot update."}, status=400)
                serializer = BillingSerializer(existing_billing, data=register_data, partial=True)
            else:
            ...
            if serializer.is_valid():
                ...
                # Ensure MongoDB directly stores testdetails and payments as native BSON arrays
                try:
                    mongo_url = os.getenv("GLOBAL_DB_HOST")
                    if mongo_url:
                        client = MongoClient(mongo_url)
                    ...
                    print("Direct MongoDB array update warning:", mongo_err)
...
def confirm_billing(request):
    ...
    try:
        with transaction.atomic():
            ...
            if franchise_id:
                is_allowed, msg = check_wallet_balance(franchise_id, new_bill_amount=billed_amount_decimal)
                if not is_allowed:
...
def cancel_tests(request):
    try:
        ...
        if billing_record.billing_status == "Pending":
            return Response({
                "error": "Cannot cancel tests when billing status is 'Pending'. Test cancellation is only available for Billed records."
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        franchise_id = request.data.get("franchise_id") or getattr(billing_record, "franchise_id", None)
        cancel_timestamp = timezone.now().isoformat()
        ...
        for test in test_details:
            ...
            if should_cancel:
                ...
                test["requested_by"] = franchise_id
                test["requested_date"] = cancel_timestamp
                test["cancelled_date"] = cancel_timestamp
                cancelled_tests.append({
                    "test_id": t_id,
                    "test_name": test.get("test_name"),
                    "status": "Cancel Requested",
                    "requested_by": franchise_id,
                    "requested_date": cancel_timestamp,
                })

            updated_test_details.append(test)

        # Save updated testdetails back as native list
        billing_record.testdetails = updated_test_details
        ...
        try:
            ...
            if mongo_url:
                ...
                coll = db["franchise_billing"]
                coll.update_one(
                    {"barcode": billing_record.barcode},
                    {"$set": {"testdetails": updated_test_details}}
                )
        except Exception as mongo_err:
            print("Direct MongoDB testdetails array update warning:", mongo_err)
...
def test_cancel_request(request):
    ...
    try:
        ...
        requester = franchise_id or billing.get("franchise_id")
        cancel_timestamp = timezone.now().isoformat()
...
def send_whatsapp_template(request):
    ...
    template = payload.get("template", "franchise_bill1")
...
def get_due_patients(request):
    ...
    from_date = request.GET.get('from_date')  # optional YYYY-MM-DD
    to_date = request.GET.get('to_date')  # optional YYYY-MM-DD
    ...
    try:
        ...
        if from_date and to_date:
            try:
                ...
                t_dt = datetime.strptime(to_date, "%Y-%m-%d") + timedelta(days=1, microseconds=-1)
                qs = qs.filter(registrationDate__gte=f_dt, registrationDate__lte=t_dt)
            except Exception:
                pass
        ...
        for bill in qs:
            ...
            payments_list = bill.payments if isinstance(bill.payments, list) else []

            # Case 1: Payment mode is "Due"
            if payment_mode == "Due":
                # Paid amount comes ONLY from non-Due payment clearance entries in payments list
            ...
            # Case 3: Paid upfront at registration (Cash, Card, UPI, etc.)
            elif payment_mode:
                paid_amount = net_amount

            due_amount = max(Decimal('0.00'), net_amount - paid_amount)
            ...
            reg_dt = bill.registrationDate
            reg_dt_str = reg_dt.isoformat() if hasattr(reg_dt, 'isoformat') else str(reg_dt)

            due_patients.append({
                "barcode": bill.barcode,
                "registrationDate": reg_dt_str,
                "registrationDateOnly": reg_dt.strftime('%Y-%m-%d') if hasattr(reg_dt, 'strftime') else (reg_dt_str[:10] if reg_dt_str else ''),
