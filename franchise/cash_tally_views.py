...
def update_due_amount(request):
    ...
    try:
        ...
        current_payments = billing.payments if isinstance(billing.payments, list) else []
...
def get_cash_tally(request):
    ...
    try:
        ...
        while current_date <= end_date:
            ...
            daily_stats.append({
                "date": current_date,
                "total_billing_amount": str(data["total_billing_amount"]),
                "franchise_share": str(data["franchise_share"]),
                "franchiser_share": str(data["franchiser_share"]),
                "collected_from_new_registrations": str(data["collected_from_new_registrations"]),
                "collected_from_past_dues": str(data["collected_from_past_dues"]),
                "total_collection_today": str(data["total_collection_today"])
            })
        ...
        total_stats_str = {k: str(v) for k, v in total_stats.items()}
...
def export_daily_tally(request):
    ...
    try:
        ...
        writer.writerow(['Date', 'Total Billing', 'Franchise Share', 'Franchiser Share', 'New Collections', 'Due Collections', 'Total Collection'])
        writer.writerow([
            target_date,
